...
function Welcome({ onNavigate }: { onNavigate: (view: string) => void }) {
  ...
  return <>
    ...
    <div className="two-column wide-left">
      <Card><div className="card-heading"><div><span className="eyebrow">Your active portion</span><h2>{data.active_portion?.title ?? "Waiting for your teacher"}</h2></div><Badge tone="amber">Class specific</Badge></div><div className="focus-card"><div className="focus-number">01</div><div><h3>{data.active_portion?.topics[0]?.title ?? "No portion published"}</h3><p>{data.active_portion?.topics[0]?.learning_outcome ?? "Your teacher will publish the next learning portion here."}</p><div className="progress-line"><i style={{ width: data.active_portion ? "42%" : "0%" }} /></div><span>{data.active_portion ? `${data.active_portion.topics.length} topics in this portion` : "Check back soon"}</span></div></div><button className="text-button" onClick={() => onNavigate("practice")} disabled={!data.active_portion}>Start portion practice <ChevronRight size={17} /></button></Card>
